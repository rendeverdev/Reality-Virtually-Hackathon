...
{
    public SteamVR_TrackedObject m_RightController;
	...
	void Update ()
	{
	    brushColor = Color.blue; //Updates our painted color with the selected color
        //var device = SteamVR_Controller.Input((int)m_RightController.index);
	...
	void DoAction(){	
		...
		if(HitTestUVPosition(ref uvWorldPosition)){
            ...
            Vector3 worldPos = brushContainer.transform.TransformPoint(uvWorldPosition);

            brushObj = PhotonNetwork.Instantiate("TexturePainter-Instances/BrushEntity", worldPos, Quaternion.identity, 0); //Paint a brush
	...
	bool HitTestUVPosition(ref Vector3 uvWorldPosition){
		RaycastHit hit;
	    Ray painterRay = RaycastHelper.Instance.GetRay();
		if (Physics.Raycast(painterRay, out hit,100,paintLayer)){
            Debug.Log("hit");
